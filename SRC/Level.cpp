...
void Level::init(string title, int width, int height) {
  ...
  // _all.add(gr5);
  // _ground.add(gr5);
  // shared_ptr<Object> gr6 (new NoLogicObject(grassWithWater, 300, 200, 60,
  // 60)); _all.add(gr6); _ground.add(gr6); shared_ptr<Object> tr1 (new
  // NoLogicObject(tree1, 100, 200, 45*4, 75*4)); _all.add(tr1);
  // _foliage.add(tr1);
...
void Level::run() {
  ...
  while (running) {
    ...
    // render.updateLayer(1, 0, _top.objects.at(0)->anState.vertices);
